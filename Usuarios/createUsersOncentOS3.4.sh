...
archivo=data/usuarios_pract4.csv
            ...
            do
                ...
                if [ ! "${VAR}" = "${group_field}" ];then
                    ...
                    if [ "${TYPE}" = "user" ];then
                        ...
                        if [ "${group_path}" = "${HOME}/${usuario}" ];
                        then
                            printf '\033[0;33m%s\033[0m\n' "    [] Usuario ${usuario} ya fue migrado a ${HOME}/${usuario}"
                        else
...
while read nombre aPaterno aMaterno dpto rol permisos fecha_ingreso db
do
    count=$((count+1))
    echo "[] Usuario (${count})"
