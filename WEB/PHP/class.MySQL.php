...
/*
 * Maintenance ed00m@gnulinuxporqueno.cl
 */
...
class MySQL {
    ...
    var $sUsername = 'usuarios';    // MySQL Username
    var $sPassword = 'usuarios';    // MySQL Password
    var $sDatabase = 'curso';    // MySQL Database
    ...
    // where $2 is true
    function Select($sFrom, $sCol='',$aWhere='', $sOrderBy='', $sLimit='', $bLike=false, $sOperand='AND', $sGroupBy=''){
        // Catch Exceptions
        if(trim($sFrom) == ''){
            return false;
        }
        ...
        if ($sCol != ''){
                    $sSQLQuery = 'SELECT '. $sCol . ' FROM `' . $sFrom . '` WHERE ';
                } else {
