...
echo "iptables -t nat -A PREROUTING -i ${ETHERWAN} -p tcp --dport ${PORTWEB} -j DNAT --to ${SERVIDORWEB}:${PORTWEB}"
#echo "iptables -t nat     -A PREROUTING -s ${IPWAN} -i ${ETHERWAN} -p tcp --destination-port ${PORTWEB} -j REDIRECT -to-port ${SERVIDORWEB}:${PORTWEB}
